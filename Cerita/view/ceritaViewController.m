...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
		...
		cerita = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, 320, self.view.frame.size.height-64)];
...
-(void)fetchCerita{
	...
	NSString *url=[NSString stringWithFormat:@"%@?json=get_recent_posts&page=%@&count=10",ApiBaseUrl,[netra getPage]];
    NSLog(@"url->%@",url);
...
-(void)email{
    ...
    [mc setMessageBody:[NSString stringWithFormat:@"<b>%@</b></br><p><a href=%@>%@</a></p></br> via Berbagi Puisi for iPhone",object_draw.Title,object_draw.url,object_draw.url] isHTML:YES];
...
-(void)twitterShare{
	...
	if([SLComposeViewController isAvailableForServiceType:SLServiceTypeTwitter]) {
        ...
        [controller setInitialText:object_draw.Title];
...
-(void)facebookShare{
	if([SLComposeViewController isAvailableForServiceType:SLServiceTypeFacebook]) {
		...
		[controller setInitialText:[NSString stringWithFormat:@"%@ via Berbagi Puisi for iPhone",object_draw.Title]];
		[controller addURL:[NSURL URLWithString:object_draw.url]];
        [self presentViewController:controller animated:YES completion:Nil];
